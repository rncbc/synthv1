		...
		m_pSynthUi(pSynthUi)
{
	...
	QDialog::setWindowIcon(QIcon(":/images/synthv1.png"));
#endif
...
void synthv1widget_config::presetsImportItems (void)
{
	synthv1_config *pConfig = synthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void synthv1widget_config::presetsExportItems (void)
{
	synthv1_config *pConfig = synthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
	...
	synthv1_presets presets;
	m_ui.PresetsTreeWidget->savePresets(&presets);
	synthv1_config::exportPresets(sFilename, &presets);
...
void synthv1widget_config::customChanged (void)
{
	++m_iDirtyCustom;
...
void synthv1widget_config::customColorThemeChanged ( int iCustomColorIndex )
{
	synthv1_config *pConfig = synthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void synthv1widget_config::customStyleThemeChanged ( int iCustomStyleIndex )
{
	synthv1_config *pConfig = synthv1_config::getInstance();
	if (pConfig == nullptr)
		return;
